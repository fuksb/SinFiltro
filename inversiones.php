  ...
  <div class="card" id="main-card">
    ...
    <div class="ganador-banner" id="ganador-banner">
      ...
      <div class="print-header">
        <div class="print-logo">SinFiltros — Informe de Inversiones</div>
...
<script>
  ...
  async function analyze() {
    ...
    try {
      const res = await fetch('inversiones-api.php', {
        method: 'POST',
  ...
  function renderResults(data) {
    ...
    if (compObj && (compObj.vs_indexado || compObj.recomendacion)) {
      ...
      compGrid.innerHTML = '';
      if (compObj.vs_indexado) {
        compGrid.innerHTML += `
          <div class="comp-row">
            <div class="comp-icon"><i class="fa fa-coins"></i></div>
            <div class="comp-text"><strong>vs Fondo indexado de referencia</strong>${escHtml(compObj.vs_indexado)}</div>
          </div>`;
      }
      if (compObj.alternativa_concreta) {
        compGrid.innerHTML += `
          <div class="comp-row">
            <div class="comp-icon"><i class="fa fa-star"></i></div>
            <div class="comp-text"><strong>Alternativa concreta recomendada</strong>${escHtml(compObj.alternativa_concreta)}</div>
          </div>`;
      }
      if (compObj.recomendacion) {
        const recBox  = document.getElementById('recomendacion-box');
        const recText = document.getElementById('recomendacion-text');
        recText.textContent = compObj.recomendacion;
