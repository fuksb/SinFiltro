...
<head>
  ...
  <link rel="canonical" href="https://sinfiltros.es/luz.php" />
  ...
  <script type="application/ld+json">
  {
    ...
    "name": "Analizador de Luz & Gas — SinFiltros",
    "url": "https://sinfiltros.es/luz.php",
    "description": "Analiza tu tarifa de luz y gas con IA. Detectamos potencia de más, cláusulas abusivas y calculamos tu coste real. Gratis.",
    ...
    @media print {
      * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
      ...
      .print-header {
        ...
        background: #92400E !important; border-bottom: none;
      }
      ...
      .result-header    { padding: 18px 22mm !important; background: #FFFBEB !important; border-bottom: 1px solid #FDE68A; page-break-after: avoid; }
      .numbers-block    { padding: 16px 22mm !important; border-bottom: 1px solid #E2E8F0; page-break-inside: avoid; }
      #potencia-section { padding: 14px 22mm 0 !important; page-break-inside: avoid; }
      .result-body      { padding: 16px 22mm 20mm !important; display: flex !important; flex-direction: column !important; gap: 18px !important; }
      .tipo-badge { background: #FEF3C7 !important; color: #92400E !important; border: 1px solid #FDE68A !important; }
      .pregunta-num { border: 2px solid #D97706 !important; color: #D97706 !important; }
      .print-footer {
    ...
    <div class="compare-tabs" id="compare-tabs" style="display:none;">
      ...
      <div class="print-header">
        <div class="print-header-left">
          ...
          <div class="print-subtitle">Informe de análisis — Luz &amp; Gas</div>
        </div>
        <div class="print-header-right">
          <div class="print-date" id="print-date"></div>
          <div class="print-url">sinfiltros.es/luz.php</div>
        </div>
      </div>
...
<script>
  ...
  async function analyze() {
    ...
    try {
      const res = await fetch('src/Api/luz-api.php', {
        method: 'POST',
  ...
  function renderResultsContent(data) {
    ...
    const sobrecoste = nums.sobrecoste_vs_pvpc_anual;
    const numItems = [
      {
      ...
      {
        val: (sobrecoste != null && sobrecoste > 0)
               ? (fmt2(sobrecoste) + '€') : (sobrecoste != null && sobrecoste <= 0) ? ('−' + fmt2(Math.abs(sobrecoste)) + '€') : null,
        lbl: sobrecoste <= 0 ? 'Ahorro vs PVPC' : 'Sobrecoste vs PVPC',
        cls: sobrecoste != null ? (sobrecoste <= 0 ? 'good' : 'cost') : 'cost'
      }
    ];
    ...
    if (comp) {
      ...
      const isNewFormat = (comp.vs_pvpc && typeof comp.vs_pvpc === 'object') || (comp.vs_competidores && typeof comp.vs_competidores === 'object');
      compGrid.className = isNewFormat ? 'comp-cards' : 'comp-grid';
      compGrid.innerHTML =
        buildCompCard(comp.vs_pvpc,         'vs PVPC regulado',                   'fa-bolt') +
        buildCompCard(comp.vs_competidores, 'vs Comercializadoras alternativas',  'fa-leaf');
