...
function fetchMortgagePage(string $url): ?string {
    // Try Puppeteer first (for JS-rendered pages)
    $puppeteerContent = scrapeWithPuppeteerSimple($url, 45);
    if ($puppeteerContent && strlen($puppeteerContent) > 250) {
        return hipCleanToText($puppeteerContent);
    }
    
    // Fallback to curl
    $cookieJar = sys_get_temp_dir() . '/sf_hip_' . md5($url) . '_' . getmypid() . '.txt';
...

{
  ...
  "resumen_oferta": "[1-2 frases directas: banco, tipo hipoteca, TIN/TAE, plazo, condiciones principales]",
  ...
  "comparativa": {
    "vs_banco_digital": {
      "diferencia_eur": [diferencia anual en € vs banco digital sin vinculaciones como Openbank/MyInvestor/EVO (positivo=esta hipoteca es más cara, negativo=más barata). null si no aplica],
      "descripcion": "[1-2 frases con cifras exactas: 'Con Openbank/MyInvestor pagarías X€ menos al año, Y€ menos en total']",
      "veredicto": "[exactamente uno de: CARO, NORMAL, BARATO, POSITIVO]"
    },
    "vs_tipo_opuesto": {
      "diferencia_eur": [diferencia estimada en coste total en € vs tipo opuesto (positivo=esta hipoteca es más cara a largo plazo). null si no calculable],
      "descripcion": "[si es FIJA: compara con variable actual (Euribor + diferencial). Si es VARIABLE: compara con fija 3.2% TIN. SIEMPRE con cifras concretas]",
      "veredicto": "[exactamente uno de: CARO, NORMAL, BARATO, POSITIVO]"
    },
