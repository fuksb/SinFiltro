...
function fetchInvestmentPage(string $url): ?string {
    $cookieJar = sys_get_temp_dir() . '/sf_inv_' . md5($url) . '_' . getmypid() . '.txt';
...
if (!$offerText && !$fileBase64A && !$aiShouldFetchUrl && !$comisionTer) {
    http_response_code(400);
...
if ($urlSource) {
    if ($aiShouldFetchUrl) {
        ...
        $offerSection = "ENLACE A ANALIZAR: {$urlSource}\n\nPor favor, visita este enlace y extrae toda la información relevante del fondo de inversión o producto de inversión (nombre, ISIN, rentabilidad histórica, comisiones, gestora, horizonte temporal recomendado, perfil de riesgo, etc.). Analiza si es una buena opción según los objetivos del inversor.\n";
    } else {
...

{
  ...
  "comparativa": {
    "vs_indexado": {
      "diferencia_eur": [exceso de comisiones en € a 20 años vs fondo indexado equivalente Amundi/iShares/Vanguard (positivo=este fondo cobra más). null si no calculable],
      "descripcion": "[1-2 frases: 'X€ en comisiones a 20 años vs Amundi/iShares/Vanguard con TER Y%. Con cifras concretas']",
      "veredicto": "[exactamente uno de: CARO, NORMAL, BARATO, POSITIVO]"
    },
    "alternativa_concreta": {
      "diferencia_eur": [ahorro estimado en comisiones a 20 años si se cambia a la alternativa recomendada (positivo=hay ahorro). null si no calculable],
      "descripcion": "[nombre fondo + ISIN + TER: 'Amundi Prime Global (LU2089238203): TER 0.05%. Ahorrarías X€ en comisiones a 20 años']",
      "veredicto": "[exactamente uno de: CARO, NORMAL, BARATO, POSITIVO]"
    },
