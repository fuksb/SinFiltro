...
if (!$offerText && !$fileBase64A && !$aiShouldFetchUrl && !$consumoKwh && !$precioKwh) {
    http_response_code(400);
...
if ($urlSource) {
    if ($aiShouldFetchUrl) {
        ...
        $offerSection = "ENLACE A ANALIZAR: {$urlSource}\n\nPor favor, visita este enlace y extrae toda la información relevante de la oferta de luz o gas (precio por kWh, potencia contratada, términos fijos, promociones, descuentos, permanencia, condiciones de facturación, etc.). Analiza si el precio es competitivo comparando con el mercado actual.\n";
    } else {
...
RESPONDE ÚNICAMENTE con un objeto JSON válido, sin markdown, sin texto antes ni después:
{
  ...
  "comparativa": {
    "vs_pvpc": {
      "diferencia_eur": [diferencia anual en € vs PVPC regulado (positivo=esta tarifa es más cara que PVPC, negativo=más barata). null si no aplica],
      "descripcion": "[1-2 frases con €/año exactos: 'Pagas X€/año más/menos que con PVPC']",
      "veredicto": "[exactamente uno de: CARO, NORMAL, BARATO, POSITIVO]"
    },
    "vs_competidores": {
      "diferencia_eur": [diferencia anual en € vs la alternativa más barata mencionada (positivo=esta tarifa es más cara). null si no aplica],
      "descripcion": "[nombres concretos y precios: 'Octopus: ~X€/mes. Holaluz: ~Y€/mes. Diferencia anual: X€']",
      "veredicto": "[exactamente uno de: CARO, NORMAL, BARATO, POSITIVO]"
    },
