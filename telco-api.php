...
function fetchTelcoPage(string $url): ?string {
    $cookieJar = sys_get_temp_dir() . '/sf_tk_' . md5($url) . '.txt';
...
if ($isUrl) {
    ...
    if ($fetched && strlen($fetched) > 250) {
        ...
        echo json_encode([
            'error'            => 'No se pudo leer el contenido del enlace automáticamente. La web del operador puede cargar con JavaScript. Copia el texto de la oferta y pégalo directamente aquí.',
...
if (!$offerText && !$fileBase64A && !$precioPromo) {
...

{
  ...
  "comparativa": {
    "vs_digi": "[Digi ofrece X con Y€/mes sin permanencia. Diferencia: Z€/mes, W€/año]",
    "vs_omv": "[Simyo/Pepephone a X€/mes. Sin permanencia. Son Y€/año menos.]",
